...
- 部署时会交互提示设置MQTT服务器地址、固件下载域名和反向代理模式（v9.0）
- 选择"使用反向代理"（默认）时，HTTP固件端口绑定 127.0.0.1，部署后自动检测并配置 Nginx Range 头透传
- 选择"不使用反向代理"时，HTTP固件端口绑定 0.0.0.0，设备直连下载
- **首次部署**: 全新安装，自动生成初始密码
...
2. **Nginx Range 头自动配置** — 实现 OTA 进度 0%→100% 实时追踪

### 13.2 反向代理模式选择

部署时脚本会询问是否使用反向代理（默认是）：
...
### 13.3 Nginx Range 头自动配置

当选择"使用反向代理"时，部署完成后脚本会自动：

1. 搜索 Nginx 配置文件（宝塔/原生/1Panel 多路径）
2. 检查 `/firmware` location 是否已配置 Range 头透传
3. 未配置时询问是否自动添加以下配置：

```nginx
# OTA-QL Range头透传（实现OTA进度0%→100%实时追踪）
...
4. 自动备份原配置 → 插入配置 → `nginx -t` 语法检查 → `nginx -s reload`

### 13.4 菜单 15：Nginx Range 头配置管理

部署后也可通过菜单 15 随时管理 Range 头配置：

| 子菜单 | 功能 |
|--------|------|
| **1. 自动配置 Range 头透传** | 检测 Nginx 配置并自动添加 4 行 Range 头透传 |
...
### 13.6 从 v8.9 升级到 v9.0

1. 更新部署脚本（wget 最新版本）
2. 运行菜单 1 重新部署，脚本会自动提示反向代理选择
3. 选择"使用反向代理"后，脚本自动检测并配置 Range 头透传
4. 也可部署后单独通过菜单 15 配置

---

> ⚡ **版本历史**:
> **v9.0** (2026-03-14) — 部署时新增反向代理模式选择（控制HTTP固件端口绑定）；Nginx Range头自动检测与配置（OTA进度0%→100%实时追踪）；新增菜单15 Nginx Range头配置管理（5项子功能）
